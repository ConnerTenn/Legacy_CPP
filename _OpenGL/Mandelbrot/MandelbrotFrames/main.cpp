...
#define IMAGE_WIDTH (4000*3)//4000
#define IMAGE_HEIGHT (2250*3)//2250
...
void main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		{/*Keyboard*/ }
		{
			...
			//static bool keyL = 0; if (GetAsyncKeyState('L')) { if (keyL == 0) { keyL = 1; PathIndex = 0; RenderPath = false; Save = 0; std::cout << "Stopped Rendering Path\n"; } } else { keyL = 0; }
			static bool keyL = 0; if (GetAsyncKeyState('L')) { if (keyL == 0) { keyL = 1; TestVideo(); } } else { keyL = 0; }
		}
		...
		{/*Render*/ }
		{
			...
			if (Save && !SaveEnable) { std::cout << "Rendering Image " << ImageCount << "\n"; }
		...
		if (Save)
		{
			if (SaveEnable)
			{
				std::cout << "Rendering and Saving Image " << ImageCount << "\n";
				{/*Uniforms*/ }
				{
					ShaderData data =
					{
				...
				glBindFramebuffer(GL_FRAMEBUFFER, FBO);
				glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
				SaveImage(IMAGE_WIDTH, IMAGE_HEIGHT, std::string(std::string(RenderPath ? "Frames/" : "") + "file" + std::to_string(ImageCount) + ".bmp").c_str(), renderedTexture);
				glBindFramebuffer(GL_FRAMEBUFFER, 0);
				glViewport(0, 0, WIDTH, HEIGHT);
				std::cout << "Done Rendering and Saving Image " << ImageCount << "\n";
			}

			if (RenderPath) { ImageCount++; }
			Save = false;
		}
	}
