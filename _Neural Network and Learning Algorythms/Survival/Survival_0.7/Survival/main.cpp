...
void Update(int threadNum)
{
	...
	while (!UpdateThreadExit)
	{
		...
		else if (*state == 3)
		{
			...
			if (cell)
			{
				if (cell->Network.ReproduceOutput->Value > 500.0 && cell->Mate)
				{
					...
					if (!cell->Mate->Alive) { CellList.push_back(new Cell(cell, cell->Mate)); }
				}
			}
		}
		else if (*state == 4)
		{
			...
			if (!proceed) { CellList.push_back(new Cell()); }
			else { CellList.push_back(new Cell(CellList[parentIndex])); }
		}
		*state = 0;
	}
}
			...
			{/*Render*/}
			{
				...
				{/*AttributeSurface*/}
				{
					if (!QuickRender)
					{
						...
						if (ActiveNeuron) { if (ActiveNeuron->Operation == 0) { operation = "Add"; } else if (ActiveNeuron->Operation == 1) { operation = "Subtract"; } else if (ActiveNeuron->Operation == 2) { operation = "Multiply"; } else if (ActiveNeuron->Operation == 3) { operation = "Divide"; } else if (ActiveNeuron->Operation == 4) { operation = "Constant"; } }
				...
				{/*ControlSurface*/}
				{
					...
					hours = int(time / 3600.0) % 60;
					days = int(minutes / 86400.0) % 24;
