...
struct EyeAttributes
{
	...
	void Mutate()
	{
		...
		if (Rand(0, MutationRate) == 0) { Offset = DRand(0.0, TAU); }
...
public:
	...
	{
		{
			...
			X = parent->X + sin(rotation) * (2 * (parent->Size + Dna.Size));
			Y = parent->Y - cos(rotation) * (2 * (parent->Size + Dna.Size));
		...
		Dna(&(parentA->Dna), &(parentB->Dna))
	{
		{
			double rotation = DRand(0, TAU);
			X = (parentA->X + parentB->X) / 2.0 + sin(rotation) * (2 * (max(parentA->Size, parentB->Size) + Dna.Size));
			Y = (parentA->Y + parentB->Y) / 2.0 - cos(rotation) * (2 * (max(parentA->Size, parentB->Size) + Dna.Size));
		}
		Rotation = DRand(0, TAU);
	...
	void Update()
	{
		...
		if (Alive)
		{
			...
			{/*Update Actions*/}
			{
				{/*Eat*/}
				{
					{/*Eat Cell*/}
					{
						if (EatCell)
						{
							...
							{
								cell = CollideList[i];
								if (Size > cell->Size * 0.4)
								{
									double before = cell->Health;
									cell->Health -= (abs(Network.EatCell->Value) * (EatPlants ? 0.2 : 1.0)) / 1000.0;
									Energy += (before - cell->Health) * 4;
					...
					{/*Eat Plants*/} 
					{
						if (EatPlants)
						{
							...
							double value = (abs(Network.EatPlants->Value) * (EatCell ? 0.2 : 1.0)) / 1000.0;
							PlantArray[x][y] -= value + 1.0;
							Energy += value / 4;
				...
				Energy -= abs(forward + back + left + right + rotationLeft + rotationRight + Size / 100.0) / 100.0;
				if (Energy <= 0.0)
				{
					Energy = 0;
					Health -= 0.2;
