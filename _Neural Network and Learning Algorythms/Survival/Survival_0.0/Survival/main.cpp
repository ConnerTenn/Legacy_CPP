	...
	while (!Done)
	{
		...
		{/*Render*/}
		{
			...
			{/*EnvironmentSurface*/}
			{
				...
				for (Cell *cell : CellList)
				{
					int x = (int)ApplyTransform((cell->X - cell->Size), EnvironmentViewportX, EnvironmentViewportScale, (double)EnvironmentSurface->Width / 2.0);//(int)((cell->X - cell->Dna.Size) * ViewportScale);
					int y = (int)ApplyTransform((cell->Y - cell->Size), EnvironmentViewportY, EnvironmentViewportScale, (double)EnvironmentSurface->Height / 2.0);//(int)((cell->Y - cell->Dna.Size) * ViewportScale);
					int width = (int)((2.0 * cell->Size) * EnvironmentViewportScale);
					int height = (int)((2.0 * cell->Size) * EnvironmentViewportScale);
					width < 2 ? width = 2 : width; height < 2 ? height = 2 : height;//Ensure Cell is always seen
					//draw rectangles when zoomed out
					if (cell == ActiveCell)
					{
						width == 2 ? EnvironmentSurface->FillRectangle(x, y, width, height, Colour(202, 81, 0)) : EnvironmentSurface->FillEllipse(x, y, width, height, Colour(202, 81, 0));
					}
					else
					{
						width == 2 ? EnvironmentSurface->FillRectangle(x, y, width, height, Colour(0, 122, 204)) : EnvironmentSurface->FillEllipse(x, y, width, height, Colour(0, 122, 204));
					}
				}
				for (Cell *cell : CellList)
				{
					if (cell == ActiveCell)
					{
						...
						int width = (int)((2.0 * cell->Size) * EnvironmentViewportScale);
						int height = (int)((2.0 * cell->Size) * EnvironmentViewportScale);
						EnvironmentSurface->FillEllipse(x + width / 2 - 10, y + height / 2 - 10, 20, 20, Colour(202, 81, 0, 255 / 2));
					}
				}
			...
			{/*AttributeSurface*/}
			{
				...
				if (ActiveCell)
				{
					for (Neuron *neuron : ActiveCell->Network.NeuronList)
					{
						int x = (int)ApplyTransform((neuron->X - 10.0), NetworkViewportX, NetworkViewportScale, (double)AttributeSurface->Width / 2.0);//(int)((neuron->X - neuron->Dna.Size) * ViewportScale);
						int y = (int)ApplyTransform((neuron->Y - 10.0), NetworkViewportY, NetworkViewportScale, (double)AttributeSurface->Height / 2.0);//(int)((neuron->Y - neuron->Dna.Size) * ViewportScale);
						int width = (int)((2.0 * 10.0) * NetworkViewportScale);
						int height = (int)((2.0 * 10.0) * NetworkViewportScale);
						width < 2 ? width = 2 : width; height < 2 ? height = 2 : height;
						if (neuron == ActiveNeuron)
						{
							AttributeSurface->FillEllipse(x, y, width, height, Colour(202, 81, 0));
						}
						else
						{
							if (neuron->Type == 1)
							{
								AttributeSurface->FillEllipse(x, y, width, height, Colour(0, 204, 50));
							}
							else if (neuron->Type == 2)
							{
								AttributeSurface->FillEllipse(x, y, width, height, Colour(204, 204, 0));
							}
							else
							{
								AttributeSurface->FillEllipse(x, y, width, height, Colour(0, 122, 204));
							}
						}
					}
				}
