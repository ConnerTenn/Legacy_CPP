...
TextBox::TextBox(XMFLOAT3 Pos = { 0.0f, 0.0f, 0.0f }, XMFLOAT2 StartPos = { 0, 0 },
	XMFLOAT2 EndPos = { 0, 0 }, vector<string> ColorMap = { "Tex\\MissingTexture.png" },
	int NumBuffers = 1, int NumColorMap = 1, string Message = "")
	:Text(Pos, StartPos, EndPos, ColorMap, NumBuffers, NumColorMap, Message)
{
...
XMMATRIX TextBox::GetTextBoxMatrix()
{
	XMMATRIX translation = XMMatrixTranslation(TextBoxPos.x, TextBoxPos.y, position_.z);
...
string TextBox::Get_Message()
{
	return Message_;
}

float TextBox::GetExpandRate()
{
	return ExpandRate_;
}

void TextBox::UpdateMessage(string Message, HRESULT* D3DResult, ID3D11Device* D3DDevice_, D3D11_SUBRESOURCE_DATA ResourceData)
{
	transform(Message.begin(), Message.end(), Message.begin(), toupper);
	if (Message != Message_)
	{
...
void TextBox::SetExpandRate(float Rate)
{
	ExpandRate_ = Rate;
}
